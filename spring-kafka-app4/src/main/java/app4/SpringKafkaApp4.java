...
public class SpringKafkaApp4 {
    ...
    static class Controller {
        ...
        @Autowired
        @Lazy
        KafkaTemplate<Long, String> longStringKafkaTemplate;
    ...
    static class Admin {
        @Bean
        public NewTopics springKafkaApp4Demo1Topic() {
            return new NewTopics(
                    TopicBuilder.name("spring-kafka-app4-demo")
                            .partitions(1)
                            .replicas(3)
                            .build(),
                    TopicBuilder.name("spring-kafka-app4-demo.DLT")
                            .partitions(1)
                            .replicas(3)
                            .build());
        }
    ...
    static class Listener {
        @KafkaListener(id = "sk-app4-demo-group", topics = "spring-kafka-app4-demo", properties = "key.deserializer:org.apache.kafka.common.serialization.LongDeserializer")
        public void listen(String in, @Header(KafkaHeaders.RECEIVED_KEY) Long key,
                @Header(KafkaHeaders.RECEIVED_PARTITION) int partition,
                @Header(KafkaHeaders.OFFSET) int offset) {
            logger.info("Data Received : {} with key {} from partition {} and offset {}.", in, key,
                    partition,
                    offset);
        ...
        @KafkaListener(id = "from.dlt", topics = "spring-kafka-app4-demo.DLT", properties = "key.deserializer:org.apache.kafka.common.serialization.LongDeserializer")
        public void listenFromDLT(String in, @Header(KafkaHeaders.RECEIVED_KEY) Long key,
                @Header(KafkaHeaders.RECEIVED_PARTITION) int partition,
                @Header(KafkaHeaders.OFFSET) int offset) {
            logger.info("DLT Data Received : {} with key {} from partition {} and offset {}.", in, key,
                    partition,
                    offset);
        }
